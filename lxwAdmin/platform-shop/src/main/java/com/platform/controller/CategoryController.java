...
public class CategoryController {
    ...
    public R list(@RequestParam Map<String, Object> params) {
        ...
        List<CategoryEntity> categoryList = categoryService.queryListing(query);
    ...
    public R save(@RequestBody CategoryEntity category) {
        Integer id1 = category.getParentId();
        //查出三级目录的名称
        List<CategoryEntity> list1 = categoryService.queryParent(id1.toString());
        for (CategoryEntity categoryEntity : list1) {
            String name=categoryEntity.getName();
            List<CategoryEntity> list2 = categoryService.queryPraentName(id1.toString());
            for (CategoryEntity categoryEntity2 : list2) {
                String names = categoryEntity2.getName();
                Integer parentIds = categoryEntity2.getParentId();
                List<CategoryEntity> list3 = categoryService.queryParentNames(parentIds.toString());
                for (CategoryEntity categoryEntity3 : list3) {
                    String namess = categoryEntity3.getName();
                    String goodstyle = namess +"--"+ names +"--" +name;
                        category.setGoodstyle(goodstyle);
     ...
     * @return
     */
    @RequestMapping("/getCategorySelecting")
    public R list1(@RequestParam Map<String, Object> params) {
        String parentId = (String) params.get("parentId");
        List<CategoryEntity>  list  = categoryService.queryLists(parentId);
        return R.ok().put("list", list);
    }
    /**
     * 查询
    ...
    public R getProduct(@RequestParam Map<String, Object> params) {
        List<CategoryEntity> list = categoryService.queryListing(params);
        return R.ok().put("list", list);
    }
}
